...
#####stm
metadata=cbind(jobs.train$level.bin,jobs.train$agency.bin)
metadata=data.frame(metadata)
out=textProcessor(jobs.train$text, metadata=metadata,lowercase=TRUE,
...
len=c()
for (i in 1:1229) {len[i]=length(word[[i]])}

kl_avg=c()
for (i in seq(5,100,by=5)){
  topics=stm(documents=docs,vocab=vocab, K=i,
               prevalence=~jobs.train$agency.bin+jobs.train$level.bin,
               content=jobs.train$agency.bin,
               data=meta)
  write.csv(topics$theta,paste("theta",i,".csv",sep=""),row.names=FALSE)
  kl=c()
  for (j in 1:6){
    M1=exp(topics$beta$logbeta[[j]])
    index=which(jobs.train$agency.bin==levels(jobs.train$agency.bin)[j])
